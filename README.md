...
- **Varde Rute** — Manage the routing/egress service `vv-llm-proxy` (the technical service id): dynamic chat-model groups (OpenRouter egress), embeddings, PII pseudonymization — applied live, and kept in sync with the LibreChat config.
...
The **Varde Rute** page (technical service `vv-llm-proxy`) manages the proxy via its admin API (`/admin/config`,
